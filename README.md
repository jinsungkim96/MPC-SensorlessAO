...
Incoming light with time-varying phase aberration $\phi_{\textrm{distort}}$ caused by atmospheric turbulence is directed to the DM with a corrected wavefront $\phi_{\textrm{cor}}$.
The reflected beam with residual wavefront aberration $\phi_{\textrm{res}} = \phi_{\textrm{distort}} + \phi_{\textrm{cor}}$ is entered onto a scientific camera, such as a CCD or CMOS, to measure the PSF.
The estimator determines the Zernike coefficient constituting $\phi_{\textrm{res}}$ without using a wavefront sensor, based on the approximate model.
